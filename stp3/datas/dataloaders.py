...
def prepare_dataloaders(cfg, return_dataset = False):
    if cfg.DATASET.NAME == 'nuscenes':
        ...
        trainloader = torch.utils.data.DataLoader(
            traindata, batch_size=cfg.BATCHSIZE, shuffle = True, num_workers = nworkers, pin_memory = False, drop_last=True
        )
        valloader = torch.utils.data.DataLoader(
            valdata, batch_size=cfg.BATCHSIZE, shuffle = False, num_workers = nworkers, pin_memory = False, drop_last = False)
